...
class InvoiceControllerTest {
  ...
  private Invoice sampleServiceInvoice() {
    ...
    List<InvoiceItem> items = List
        .of(InvoiceItem.restore(1L, "Freno delantero", BigDecimal.ONE, new BigDecimal("36900.00")));
  ...
  private Invoice sampleDeliveryInvoice() {
    List<InvoiceItem> items = List
        .of(InvoiceItem.restore(1L, "Boxer 150 Palanca de freno", new BigDecimal("2"), new BigDecimal("18500.00")));
    return Invoice.restore(2L, InvoiceType.DELIVERY, null, null, "Talleres La 80", LocalDateTime.now(), BigDecimal.ZERO,
        false, items);
  }
