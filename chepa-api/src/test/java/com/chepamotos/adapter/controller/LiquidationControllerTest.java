...
class LiquidationControllerTest {
  ...
  @BeforeEach
  void resetMocks() {
    Mockito.reset(liquidationApplicationService);
  }

  @Test
  void list_returnsOkEnvelope() throws Exception {
    when(liquidationApplicationService.list(any(), any()))
        .thenReturn(List.of(sampleLiquidation(1L, 1L, "Jose")));
  ...
  void create_singleMechanic_whenValid_returnsCreatedEnvelope() throws Exception {
    when(liquidationApplicationService.create(any(), anyLong()))
        .thenReturn(List.of(sampleLiquidation(10L, 1L, "Jose")));
  ...
  void create_allMechanics_whenValid_returnsCreatedEnvelope() throws Exception {
    when(liquidationApplicationService.create(any(), any())).thenReturn(List.of(
  ...
  void create_whenLiquidationAlreadyExists_returns409WithStandardError() throws Exception {
    when(liquidationApplicationService.create(any(), anyLong()))
        .thenThrow(new LiquidationAlreadyExistsException(1L, LocalDate.of(2026, 1, 28)));
